...
const DeleteVideosForm = ({ mediaId, data }: { mediaId: number, data: Media }) => {
    const t = useTranslations("ui.forms.crud.videos.delete");
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            await FetchDataAxios({
                ...
                console.log(r);
                setDataToast({type: "success", message: t("messages.success") ?? "The video info has been deleted successfully!", statusToast: true});
                ...
                setDataToast({type: "error", message: t("messages.error", {message: ""+err}) ?? `Failed to delete this video info! Message: ${err}`, statusToast: true});
            });
        } catch (error) {
            setDataToast({type: "error", message: t("messages.errorapi", {message: ""+error}) ?? `Ocurred an error while deleting this video info! Message: ${error}`, statusToast: true});
        }
    }
    ...
    return (
        <div className="container">
            ...
            {!!isLoggedIn && (
                <>
                    {dataToast.statusToast && <Toasts id={"toastDelVideos"} data={dataToast} modeType={1} />}

                    <h3 className="title mx-auto text-center">
                        {t("title") ?? "Delete Videos"}
                    </h3>
                    <form className={styles.frmdeletevideos}>
                                ...
                                <table className="table table-nobordered table-rounded table-autolayout">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{t("table.header.privacy") ?? "Privacy"}</th>
                                            <th>{t("table.header.isFeatured") ?? "Is featured?"}</th>
                        ...
                        <p className="text-center mx-auto mt-3">
                            {t("messages.confirm.title", {mediaId: ""+data.mediaId}) ?? "Are you sure you want to delete this video info (id: {mediaId})?"}
                        </p>
